...
! real (kind=8),DIMENSION(nLayers) :: speciesIDs
 ...
 real (kind=8) :: D13m2(nLayers),BAm2(nLayers),Nm2(nLayers),hm2(nLayers),dNm2(nLayers), dBAm2(nLayers), rfm2(nLayers)
...
!!!!Calculate mortality with Mikko's model
D13m2 = STAND_all(12,:)
BAm2 = STAND_all(13,:)/STAND_all(17,:) 
Nm2 = STAND_all(17,:)
hm2 = STAND_all(11,:)
dNm2 = 0.
dBAm2 = 0.
...
! Mortality - use Mikko's model
     if(time==inttimes) then
	  ...
	  if (dN<0. .and. Nold>0.) then
			...
			W_crh = stand(54)
			W_stem = stand(31)
	S_branch = STAND(26) + (W_branch + W_croot*0.83 + Wdb) * min(1.,-dN*step/Nold) 
	S_wood = STAND(29)+(W_croot*0.17 + W_stem) * min(1.,-dN*step/Nold)
    S_fol = STAND(26) + wf_STKG * min(1.,-dN*step/Nold) !foliage litterfall
    S_fr  = STAND(27) + W_froot * min(1.,-dN*step/Nold)  !fine root litter
			W_wsap = W_wsap * N/Nold
			W_froot = W_froot * N/Nold
			W_c = W_c * N/Nold
			W_s = W_s * N/Nold
